        ...
        <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] lg:p-8">
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl font-black tracking-[-0.05em] text-slate-900 sm:text-4xl">Manage orders, products, and buyers from one admin hub.</h1>
              <p className="max-w-2xl text-sm leading-6 text-stone-600 sm:text-base">
                This view is driven by your live Supabase orders, buyers, and product records.
              </p>
            </div>
            <div className="flex flex-col gap-3 sm:flex-row">
              <Button variant="industrial" className="h-11 rounded-2xl px-5">Export Report</Button>
              <Button variant="outline" className="h-11 rounded-2xl border-stone-300 bg-white/80 px-5">
                Platform Status: {data.billingHealthLabel}
              </Button>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                { label: 'Review SLA', value: data.reviewSlaLabel, icon: Clock3 },
                { label: 'Catalog health', value: data.billingHealthLabel, icon: ShieldAlert },
                { label: 'Buyer growth', value: data.buyerTrend, icon: TrendingUp },
              ].map((item) => (
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.8fr)]">
        ...
        <div className="rounded-[2rem] border border-stone-200/80 bg-white/90 p-5 shadow-sm sm:p-6">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-black tracking-tight text-slate-900">Revenue Performance</h2>
              <p className="mt-1 text-sm text-stone-600">Revenue movement across recent order cycles.</p>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-[2rem] border border-stone-200/80 bg-white/90 p-5 shadow-sm sm:p-6">
            ...
            <div className="mt-6 space-y-3">
              {[
                { title: 'Product Reviews', count: data.actionRequired.productModeration, color: 'bg-amber-500', href: '/admin/products' },
                { title: 'Order Queue', count: data.actionRequired.planManagement, color: 'bg-emerald-500', href: '/admin/orders' },
              ].map((task) => (
          ...
          <div className="relative overflow-hidden rounded-[2rem] bg-workshop-dark p-5 text-white shadow-sm sm:p-6">
            ...
            <div className="relative z-10">
              ...
              <p className="mt-1 text-sm text-white/70">Derived from live store, order, and product state.</p>
